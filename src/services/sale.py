...
from sqlalchemy import select, insert, func, or_, and_, case, Float, cast, update, String, Numeric
...
from src.db.models import (
    ...
    Distributor, SKU, Pharmacy, Brand, ProductGroup, PromotionType, 
    Employee, Settlement, District, ImportLogs, Segment, GeoIndicator
)
from src.utils.excel_parser import parse_excel_file
from src.utils.mapping import map_record
from src.mapping.sales import primary_sales_mapping, secondary_sales_mapping, tertiary_sales_mapping
...
class PrimarySalesAndStockService(BaseService[PrimarySalesAndStock, sale.PrimarySalesAndStockCreate, sale.PrimarySalesAndStockUpdate]):
    async def import_sales(self, session: 'AsyncSession', file: 'UploadFile', user_id: int):
        ...
        distributor_names = {r['дистрибьютор'] for r in records}
        sku_names = {r['sku'] for r in records}
        ...
        for idx, r in enumerate(records):
            ...
            if r['дистрибьютор'] in missing_distributors:
                missing_keys.append(f"дистрибьютор: {r['дистрибьютор']}")

            if r['sku'] in missing_skus:
                missing_keys.append(f"SKU: {r['sku']}")
            ...
            relation_fields = {
                'distributor_id': distributor_map[r['дистрибьютор']],
                'sku_id': sku_map[r['sku']],
                'import_log_id': import_log.id,
            }
            data_to_insert.append(map_record(r, primary_sales_mapping, relation_fields))
    ...
    async def get_shipment_stock_report(
        ...
        if filters.quarters:
            base_stmt = base_stmt.where(PrimarySalesAndStock.quarter.in_(filters.quarters))
                ...
                *final_select_fields,
                func.json_object_agg(
                    ...
                    func.json_build_object(
                        'packages', base_stmt.c.packages,
                        'amount', base_stmt.c.amount
                    )
                ).label('periods_data')
            )
        )
    ...
    async def get_period_totals(
        ...
        if filters.group_by_period == 'quarter':
            sales_divisor = func.nullif(
                func.sum(
                    case((PrimarySalesAndStock.indicator == 'Первичная продажа',
                          PrimarySalesAndStock.packages), else_=0)
                ) / 3.0,
                0
            )
        ...
        select_cols = [
            ...
            func.cast(case(
                (func.sum(
                    case((PrimarySalesAndStock.indicator == 'Первичная продажа',
                          PrimarySalesAndStock.packages), else_=0)) > 0,
                 func.sum(
                     case((PrimarySalesAndStock.indicator == 'Остаток на складе',
                           PrimarySalesAndStock.packages), else_=0)) / sales_divisor),
                else_=None
            ), Numeric(10, 2)).label('coverage_months'),
        ]
        ...
        if filters.group_by_period not in ('quarter', 'year'):
            select_cols.extend([
                ...
                func.round(func.sum(
                    case((PrimarySalesAndStock.indicator == 'Остаток на складе',
                          PrimarySalesAndStock.amount), else_=0)
                )).label('stock_amount'),
            ])
        ...
        stmt = (
            ...
            .join(SKU, PrimarySalesAndStock.sku_id == SKU.id)
            .where(
                PrimarySalesAndStock.indicator.in_(['Остаток на складе', 'Первичная продажа']),
                PrimarySalesAndStock.year.in_(filters.years),
            )
        )
        ...
        if filters.distributor_ids:
            stmt = stmt.where(PrimarySalesAndStock.distributor_id.in_(filters.distributor_ids))
        ...
        base_stmt = (
            select(
                *select_fields,
                ...
                func.cast(case(
                    (func.sum(
                        case((PrimarySalesAndStock.indicator == 'Первичная продажа', PrimarySalesAndStock.packages),
                             else_=0)) > 0,
                     func.sum(
                         case((PrimarySalesAndStock.indicator == 'Остаток на складе', PrimarySalesAndStock.packages),
                              else_=0)) /
                     func.nullif(func.sum(
                         case((PrimarySalesAndStock.indicator == 'Первичная продажа', PrimarySalesAndStock.packages),
                              else_=0)), 0)),
                    else_=0
                ), Numeric(10, 2)).label('coverage_months')
            ...
            select(
                *final_select_fields,
                func.json_object_agg(
                    base_stmt.c.period,
                    func.json_build_object(
                        'coverage_months', base_stmt.c.coverage_months
                    )
                ).label('periods_data')
            )
        )
            ...
            select(
                *final_select_fields,
                func.json_object_agg(
                    ...
                    func.json_build_object(
                        'amount', with_percentages.c.amount,
                        'share_percent', with_percentages.c.share_percent
                    )
                ).label('periods_data')
            )
        )
    ...
    async def get_distributor_share_chart(
        ...
        if filters.months:
            period_totals = period_totals.where(PrimarySalesAndStock.month.in_(filters.months))

        if filters.quarters:
            period_totals = period_totals.where(PrimarySalesAndStock.quarter.in_(filters.quarters))
    ...
    @staticmethod
    async def get_unpublish(session: 'AsyncSession', limit=100, offset=0) -> Sequence[ModelType]:
        stmt = select(PrimarySalesAndStock).where(PrimarySalesAndStock.published.is_(False))
...
class SecondarySalesService(BaseService[SecondarySales, sale.SecondarySalesCreate, sale.SecondarySalesUpdate]):
    async def import_sales(self, session: 'AsyncSession', file: 'UploadFile', user_id: int):
        records = await parse_excel_file(file)
        ...
        pharmacy_names = {r['аптека'] for r in records}
        sku_names = {r['sku'] for r in records}
        ...
        for idx, r in enumerate(records):
            ...
            if r['аптека'] in missing_pharmacies:
                missing_keys.append(f"аптека: {r['аптека']}")
            ...
            relation_fields = {
                'pharmacy_id': pharmacy_map[r['аптека']],
                'sku_id': sku_map[r['sku']],
                'import_log_id': import_log.id,
            }
            data_to_insert.append(map_record(r, secondary_sales_mapping, relation_fields))
            ...
            select(
                *final_select_fields,
                func.json_object_agg(
                    base_stmt.c.period,
                    func.json_build_object(
                        'total_packages', base_stmt.c.total_packages,
                        'total_amount', base_stmt.c.total_amount
                    )
                ).label('periods_data')
            )
        )
    ...
    async def get_total_sales_by_distributor(
        ...
        if filters.product_group_ids:
            base_stmt = base_stmt.where(SKU.product_group_id.in_(filters.product_group_ids))

        if filters.geo_indicator_ids:
            base_stmt = base_stmt.where(Pharmacy.geo_indicator_id.in_(filters.geo_indicator_ids))
...
class TertiarySalesService(BaseService[TertiarySalesAndStock, sale.TertiarySalesCreate, sale.TertiarySalesUpdate]):
    async def import_sales(self, session: 'AsyncSession', file: 'UploadFile', user_id: int):
        records = await parse_excel_file(file)
    ...
    async def get_sales_report(
        ...
        if filters.quarters:
            base_stmt = base_stmt.where(TertiarySalesAndStock.quarter.in_(filters.quarters))
    ...
    async def get_period_totals(
        ...
        sales_packages = func.sum(
            case((TertiarySalesAndStock.indicator == 'Третичные продажи',
                  TertiarySalesAndStock.packages), else_=0)
        )
        sales_amount = func.sum(
            case((TertiarySalesAndStock.indicator == 'Третичные продажи',
                  TertiarySalesAndStock.amount), else_=0)
        )
    ...
    async def get_numeric_distribution(
        ...
        if filters.months:
            total_pharmacies_cte = total_pharmacies_cte.where(TertiarySalesAndStock.month.in_(filters.months))
        if filters.quarters:
            total_pharmacies_cte = total_pharmacies_cte.where(TertiarySalesAndStock.quarter.in_(filters.quarters))

        total_pharmacies_cte = total_pharmacies_cte.group_by(
            Pharmacy.distributor_id,
        ...
        base_stmt = (
            select(
                *select_fields,
                ...
                func.cast((func.count(func.distinct(TertiarySalesAndStock.pharmacy_id)).cast(Float) /
                           func.nullif(total_pharmacies_cte.c.total_pharmacies, 0) * 100), Numeric(10, 2)).label(
                    'nd_percent')
            )
            .select_from(TertiarySalesAndStock)
            .join(SKU, TertiarySalesAndStock.sku_id == SKU.id)
                ...
                and_(
                    ...
                    or_(
                        and_(
                            ...
                            Pharmacy.geo_indicator_id == total_pharmacies_cte.c.geo_indicator_id
                        ),
                        and_(
        ...
        group_by_fields.extend(
            [Distributor.id, Pharmacy.geo_indicator_id, period_key, total_pharmacies_cte.c.total_pharmacies])
        base_stmt = base_stmt.group_by(*group_by_fields).cte('period_agg')
            ...
            select(
                *final_select_fields,
                func.json_object_agg(
                    base_stmt.c.period,
                    func.json_build_object(
                        'nd_percent', base_stmt.c.nd_percent
                    )
                ).label('periods_data')
            )
        )
    ...
    async def get_low_stock(
        ...
        base_stmt = (
            select(
                *select_fields,
                period_key.label('period'),
                func.cast(func.sum(TertiarySalesAndStock.packages).label('total_packages'), Numeric(10, 2))
            )
            .select_from(TertiarySalesAndStock)
            .join(SKU, TertiarySalesAndStock.sku_id == SKU.id)
            .join(Brand, SKU.brand_id == Brand.id)
        ...
        if filters.responsible_employee_ids:
            base_stmt = base_stmt.where(Employee.id.in_(filters.responsible_employee_ids))
        ...
        if filters.search and filters.group_by_dimensions:
            ...
            if 'responsible_employee' in filters.group_by_dimensions:
                search_conditions.append(
                    Employee.full_name.ilike(search_term) if Employee.full_name is not None else False
                )
        ...
        group_by_fields.append(period_key)
        base_stmt = base_stmt.group_by(*group_by_fields).having(
            and_(
