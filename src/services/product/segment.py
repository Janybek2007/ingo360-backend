...
class SegmentService(
    BaseService[
        products.Segment, product_schema.SegmentCreate, product_schema.SegmentUpdate
    ]
):
    async def get_multi(
        self,
        session: "AsyncSession",
        filters: product_schema.SegmentListRequest | None = None,
        load_options: list[Any] | None = None,
    ) -> PaginatedResponse[product_schema.SegmentResponse]:
    ...
    async def import_excel(
        ...
        validate_required_columns(records, product.segment_fields)
        ...
        await resolve_records_fields(
            session, records, product.segment_fields, self.get_id_map
        )
