...
from sqlalchemy import select, func, insert, literal, union, case, or_, and_, distinct

from src.services.base import BaseService
from src.db.models import IMS, ImportLogs, Company, Brand
from src.utils.mapping import map_record
from src.utils.excel_parser import parse_excel_file
from src.mapping.ims import ims_mapping
from src.schemas.ims import IMSCreate, IMSUpdate, IMSResponse, IMSTopFilter, IMSMetricsFilter, IMSMetricsResponse, IMSTableFilter
    ...
    async def get_entities_with_metrics(
        ...
        ranked_subquery = (
            select(
                ...
                func.row_number().over(
                    order_by=func.sum(IMS.amount).desc()
                ).label('rank')
            )
        ...
        if company_id is not None and filters.group_column == "company":
            company_name_stmt = select(
                Company.ims_name.label('ims_name'),
            ...
            if company_ims_name:
                ...
                brands_stmt = select(Brand.ims_name).where(Brand.company_id == company_id)
            ...
            stmt = (
                select(
                    ...
                    ranked_subquery.c.sales,
                    case(
                        {func.upper(ranked_subquery.c.entity) == func.upper(filters.segment_name): True},
                        else_=False
            ...
            stmt = (
                select(
                    ...
                    ranked_subquery.c.sales,
                    case(
                        {func.upper(ranked_subquery.c.entity) == func.upper(filters.brand_name): True},
                        else_=False
                    ).label('is_user_entity')
                )
                .order_by(ranked_subquery.c.rank)
        ...
        previous_periods = self._get_previous_periods(filters.periods, filters.type_period)
        ...
        prev_market_sales_stmt = (
            select(func.sum(IMS.amount))
            .where(IMS.period.in_(previous_periods))
        )
        ...
        growth_vs_previous = (
            ...
            if prev_entity_sales > 0 else 0.0
        )

        market_growth = (
            ((market_sales - prev_market_sales) / prev_market_sales * 100)
            if prev_market_sales > 0 else 0.0
        )
    ...
    async def get_table_data(
        ...
        if 'periods' in filters.model_fields_set:
            for period in filters.periods:
                ...
                period_amount = func.round(func.sum(
                    case(
                        (IMS.period.in_(db_periods), IMS.amount),
                        else_=0
                    )
                ))

                period_packages = func.round(func.sum(
                    case(
                        (IMS.period.in_(db_periods), IMS.packages),
                        else_=0
                    )
                ))
