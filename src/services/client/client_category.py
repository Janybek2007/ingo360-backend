...
class ClientCategoryService(
    BaseService[
        ...
        client_schema.ClientCategoryCreate,
        client_schema.ClientCategoryUpdate,
    ]
):
    async def get_multi(
        self,
        session: "AsyncSession",
        filters: client_schema.ClientCategoryListRequest | None = None,
        load_options: list[Any] | None = None,
    ) -> PaginatedResponse[client_schema.ClientCategoryResponse]:
    ...
    async def iter_multi(
        ...
        load_options: list[Any] | None = None,
        chunk_size: int = 1000,
    ) -> AsyncIterator[clients.ClientCategory]:
        stmt = select(self.model)
    ...
    async def import_excel(
        ...
        validate_required_columns(records, client.client_category_fields)
        ...
        await resolve_records_fields(
            session, records, client.client_category_fields, self.get_id_map
        )
