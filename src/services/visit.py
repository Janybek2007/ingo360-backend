...
from sqlalchemy import insert, select, func, or_, and_, String

from .base import BaseService
from src.db.models import (
    Visit, ProductGroup, ClientCategory, Doctor,
    Employee, MedicalFacility, Speciality, ImportLogs, Pharmacy,
    GeoIndicator,
)
...
class VisitService(BaseService[Visit, visit.VisitCreate, visit.VisitUpdate]):
    async def import_sales(self, session: 'AsyncSession', file: 'UploadFile', user_id: int):
        ...
        results = await asyncio.gather(
            ...
            self.get_id_map(session, Employee, 'full_name', {r['сотрудник'] for r in records}),
        ...
        doctor_map, missing_doctors = await self.get_id_map(
            session, Doctor, 'full_name', doctor_values
        ) if doctor_values else ({}, set())
        ...
        ) if medical_facilities else ({}, set())
        ...
        pharmacy_map, missing_pharmacies = await self.get_id_map(
            session, Pharmacy, 'name', pharmacies
        ) if pharmacies else ({}, set())
        ...
        for idx, r in enumerate(records):
            ...
            if r.get('тип клиента') == 'Врач':
                if r.get('учреждение') and r['учреждение'] in missing_medical_facilities:
    ...
    async def get_doctor_count_by_speciality(
        ...
        stmt = (
            select(
                ...
                func.coalesce(doctors_with_visits_subquery.c.doctors_with_visits, 0).label('count_with_visits'),
                (
                        func.coalesce(doctors_with_visits_subquery.c.doctors_with_visits, 0) * 100.0 /
        ...
        if filters.medical_facility_ids:
            stmt = stmt.where(Doctor.medical_facility_id.in_(filters.medical_facility_ids))
        if filters.search:
        ...
        return [
            visit.DoctorsBySpecialtyResponse(**row)
    ...
    async def get_doctor_count_with_visits_by_speciality(
        ...
        if filters.speciality_ids:
            all_doctors_subquery = all_doctors_subquery.where(Speciality.id.in_(filters.speciality_ids))
        if filters.medical_facility_ids:
            all_doctors_subquery = all_doctors_subquery.where(MedicalFacility.id.in_(filters.medical_facility_ids))
        ...
        if filters.search:
            ...
            if search_conditions:
                all_doctors_subquery = all_doctors_subquery.where(or_(*search_conditions))

        all_doctors_subquery = all_doctors_subquery.group_by(*group_by_fields).subquery()
        ...
        for dim in filters.group_by_dimensions:
            if dim == 'medical_facility':
                visit_select_fields.append(MedicalFacility.id.label('medical_facility_id'))
        ...
        if company_id:
            doctors_with_visits_subquery = doctors_with_visits_subquery.where(Employee.company_id == company_id)
        if filters.speciality_ids:
            doctors_with_visits_subquery = doctors_with_visits_subquery.where(Speciality.id.in_(filters.speciality_ids))
        if filters.medical_facility_ids:
            doctors_with_visits_subquery = doctors_with_visits_subquery.where(
                MedicalFacility.id.in_(filters.medical_facility_ids))
        if filters.doctor_ids:
            doctors_with_visits_subquery = doctors_with_visits_subquery.where(Doctor.id.in_(filters.doctor_ids))
        ...
        if filters.search:
            ...
            if search_conditions:
                doctors_with_visits_subquery = doctors_with_visits_subquery.where(or_(*search_conditions))

        doctors_with_visits_subquery = doctors_with_visits_subquery.group_by(*visit_group_by_fields).subquery()

        join_conditions = []
        for dim in filters.group_by_dimensions:
            if dim == 'medical_facility':
                join_conditions.append(
                    all_doctors_subquery.c.medical_facility_id == doctors_with_visits_subquery.c.medical_facility_id
                )
            elif dim == 'speciality':
                join_conditions.append(
                    all_doctors_subquery.c.speciality_id == doctors_with_visits_subquery.c.speciality_id
                )
            elif dim == 'doctor':
                join_conditions.append(
                    all_doctors_subquery.c.doctor_id == doctors_with_visits_subquery.c.doctor_id
                )
        ...
        stmt = (
            select(
                *final_select_fields,
                func.coalesce(doctors_with_visits_subquery.c.doctors_with_visits, 0).label('doctors_with_visits'),
    ...
    async def get_visits_sum_for_period(
        ...
        for dim in filters.group_by_dimensions:
            ...
            if dim_config['id_field'] is not None:
                select_fields.append(dim_config['id_field'].label(dim_config['id_label']))
                group_by_fields.append(dim_config['id_field'])

            if dim_config['name_field'] is not None:
                select_fields.append(dim_config['name_field'].label(dim_config['name_label']))
        ...
        join_order = ['employee', 'product_group', 'pharmacy', 'medical_facility', 'doctor', 'speciality',
                      'geo_indicator']
        ...
        if filters.medical_facility_ids:
            stmt = stmt.where(Visit.medical_facility_id.in_(filters.medical_facility_ids))
    ...
    async def get_visits_by_period(
            session: 'AsyncSession',
            filters: visit.VisitCountFilter,
            company_id: int | None
    ):
