...
from src.schemas.user import UserCreate, UserUpdate, UserCreateWithoutPassword, UserAdminUpdate
...
class UserManager(IntegerIDMixin, BaseUserManager[User, int]):
    ...
    async def on_after_request_verify(
        ...
        log.info('Verification requested for user %r. Verification token: %r', user.id, token)
    ...
    async def create_with_permission(
        ...
        await self.send_password_setup_email(created_user.email, token, created_user.first_name)
    ...
    async def update(
        ...
        if 'password' in update_data:
            update_data['hashed_password'] = self.password_helper.hash(update_data['password'])
    ...
    @staticmethod
    async def get_user_by_id(session: 'AsyncSession', user_id: int, load_options: list[Any] | None = None) -> User:
        stmt = select(User)
    ...
    @staticmethod
    async def get_regular_users(session: 'AsyncSession', load_options: list[Any] | None = None) -> Sequence[User]:
        stmt = select(User)
    ...
    @staticmethod
    async def get_admins_and_operators(session: 'AsyncSession', load_options: list[Any] | None = None) -> Sequence[User]:
        stmt = select(User)
    ...
    async def send_password_setup_email(self, email: str, token: str, user_name: str | None = None):
        ...
        if self.background_tasks:
            self.background_tasks.add_task(
                ...
                to_email=email,
                body=html_body,
                subject=f'Установите пароль для вашего аккаунта',
                is_html=True
            )
    ...
    async def validate_password_setup_token(
        ...
        stmt = select(PasswordSetupToken).where(
            ...
            PasswordSetupToken.is_used == False
        )
        result = await session.execute(stmt)
    ...
    async def resend_password_setup(
        ...
        stmt = update(PasswordSetupToken).where(
    ...
    async def get_all(self, session: 'AsyncSession', filters: UserFilter, load_options: list[Any] | None = None):
        stmt = select(User)
    ...
    async def get_clients(self, session: 'AsyncSession', filters: UserFilter, load_options: list[Any] | None = None):
