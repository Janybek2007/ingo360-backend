...
if TYPE_CHECKING:
    from . import Brand, Employee, User, Pharmacy, District, SKU, ProductGroup, ImportLogs
...
class Company(Base):
    ...
    product_groups: Mapped[list['ProductGroup']] = relationship(back_populates='company')
    import_log_id: Mapped[int | None] = mapped_column(ForeignKey('import_logs.id', ondelete='CASCADE'), nullable=True)
    import_log: Mapped[Optional['ImportLogs']] = relationship(back_populates='companies')
