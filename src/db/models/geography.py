...
class Country(Base):
    ...
    import_log: Mapped[Optional['ImportLogs']] = relationship(back_populates='countries')
...
class Settlement(Base):
    ...
    districts: Mapped[list['District']] = relationship(back_populates='settlement', foreign_keys='[District.settlement_id]')
    medical_facilities: Mapped[list['MedicalFacility']] = relationship(back_populates='settlement')
    pharmacies: Mapped[list['Pharmacy']] = relationship(back_populates='settlement')
    import_log_id: Mapped[int | None] = mapped_column(ForeignKey('import_logs.id', ondelete='CASCADE'), nullable=True)
    import_log: Mapped[Optional['ImportLogs']] = relationship(back_populates='settlements')
...
class District(Base):
    ...
    settlement: Mapped[Optional['Settlement']] = relationship(back_populates='districts', foreign_keys='[District.settlement_id]')
    employees: Mapped[list['Employee']] = relationship(back_populates='district', foreign_keys='[Employee.district_id]')
    medical_facilities: Mapped[list['MedicalFacility']] = relationship(back_populates='district')
    ...
    import_log_id: Mapped[int | None] = mapped_column(ForeignKey('import_logs.id', ondelete='CASCADE'), nullable=True)
    import_log: Mapped[Optional['ImportLogs']] = relationship(back_populates='districts')
