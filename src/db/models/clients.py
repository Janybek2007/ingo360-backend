...
if TYPE_CHECKING:
    from . import (
        Employee, Settlement, District, Base, Company, Visit,
        PrimarySalesAndStock, SecondarySales, TertiarySalesAndStock,
        ProductGroup, ImportLogs, Region
    )
...
class ClientCategory(Base):
    ...
    pharmacies: Mapped[list['Pharmacy']] = relationship(back_populates='client_category')
    import_log_id: Mapped[int | None] = mapped_column(ForeignKey('import_logs.id', ondelete='CASCADE'), nullable=True)
    import_log: Mapped[Optional['ImportLogs']] = relationship(back_populates='client_categories')
...
class Doctor(Base):
    ...
    responsible_employee: Mapped[Optional['Employee']] = relationship(back_populates='doctors')
    medical_facility_id: Mapped[int | None] = mapped_column(ForeignKey('medical_facilities.id'), nullable=True)
    medical_facility: Mapped[Optional['MedicalFacility']] = relationship(back_populates='doctors')
    ...
    product_group: Mapped[Optional['ProductGroup']] = relationship(back_populates='doctors')
    import_log_id: Mapped[int | None] = mapped_column(ForeignKey('import_logs.id', ondelete='CASCADE'), nullable=True)
...
class Pharmacy(Base):
    ...
    distributor: Mapped[Optional['Distributor']] = relationship(back_populates='pharmacies')
    responsible_employee_id: Mapped[int | None] = mapped_column(ForeignKey('employees.id'), nullable=True)
    responsible_employee: Mapped[Optional['Employee']] = relationship(back_populates='pharmacies')
    settlement_id: Mapped[int | None] = mapped_column(ForeignKey('settlements.id'), nullable=True)
    settlement: Mapped[Optional['Settlement']] = relationship(back_populates='pharmacies')
    ...
    client_category_id: Mapped[int | None] = mapped_column(ForeignKey('client_categories.id'), nullable=True)
    client_category: Mapped[Optional['ClientCategory']] = relationship(back_populates='pharmacies')
    secondary_sales: Mapped[list['SecondarySales']] = relationship(back_populates='pharmacy')
    tertiary_sales: Mapped[list['TertiarySalesAndStock']] = relationship(back_populates='pharmacy')
    ...
    import_log_id: Mapped[int | None] = mapped_column(ForeignKey('import_logs.id', ondelete='CASCADE'), nullable=True)
    import_log: Mapped[Optional['ImportLogs']] = relationship(back_populates='pharmacies')
    visits: Mapped[list['Visit']] = relationship(back_populates='pharmacy')
    geo_indicator_id: Mapped[int | None] = mapped_column(ForeignKey('geo_indicators.id'), nullable=True)
    geo_indicator: Mapped[Optional['GeoIndicator']] = relationship(back_populates='pharmacies')
...
class Speciality(Base):
    ...
    import_log_id: Mapped[int | None] = mapped_column(ForeignKey('import_logs.id', ondelete='CASCADE'), nullable=True)
    import_log: Mapped[Optional['ImportLogs']] = relationship(back_populates='specialities')
...
class MedicalFacility(Base):
    ...
    district_id: Mapped[int | None] = mapped_column(ForeignKey('districts.id'), nullable=True)
    district: Mapped[Optional['District']] = relationship(back_populates='medical_facilities')
    ...
    import_log_id: Mapped[int | None] = mapped_column(ForeignKey('import_logs.id', ondelete='CASCADE'), nullable=True)
    import_log: Mapped[Optional['ImportLogs']] = relationship(back_populates='medical_facilities')
    facility_type: Mapped[str | None] = mapped_column(String(256), nullable=True)
    geo_indicator_id: Mapped[int | None] = mapped_column(ForeignKey('geo_indicators.id'), nullable=True)
    geo_indicator: Mapped[Optional['GeoIndicator']] = relationship(back_populates='medical_facilities')
...
class Distributor(Base):
    ...
    primary_sales: Mapped[list['PrimarySalesAndStock']] = relationship(back_populates='distributor')
    import_log_id: Mapped[int | None] = mapped_column(ForeignKey('import_logs.id', ondelete='CASCADE'), nullable=True)
    import_log: Mapped[Optional['ImportLogs']] = relationship(back_populates='distributors')
...
class GeoIndicator(Base):
    ...
    medical_facilities: Mapped[list['MedicalFacility']] = relationship(back_populates='geo_indicator')
    import_log_id: Mapped[int | None] = mapped_column(ForeignKey('import_logs.id', ondelete='CASCADE'), nullable=True)
    import_log: Mapped[Optional['ImportLogs']] = relationship(back_populates='geo_indicators')
