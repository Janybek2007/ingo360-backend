...
if TYPE_CHECKING:
    from . import (
        Employee, Company, Visit, PrimarySalesAndStock,
        SecondarySales, TertiarySalesAndStock, Pharmacy,
        Doctor, ImportLogs
    )
...
class DosageForm(Base):
    ...
    import_log_id: Mapped[int | None] = mapped_column(ForeignKey('import_logs.id', ondelete='CASCADE'), nullable=True)
    import_log: Mapped[Optional['ImportLogs']] = relationship(back_populates='dosage_forms')
...
class SKU(Base):
    ...
    dosage_id: Mapped[int | None] = mapped_column(ForeignKey('dosages.id'), nullable=True)
    ...
    primary_sales: Mapped[list['PrimarySalesAndStock']] = relationship(back_populates='sku')
    secondary_sales: Mapped[list['SecondarySales']] = relationship(back_populates='sku')
    tertiary_sales: Mapped[list['TertiarySalesAndStock']] = relationship(back_populates='sku')
    company_id: Mapped[int] = mapped_column(ForeignKey('companies.id'))
...
class ProductGroup(Base):
    ...
    import_log_id: Mapped[int | None] = mapped_column(ForeignKey('import_logs.id', ondelete='CASCADE'), nullable=True)
    import_log: Mapped[Optional['ImportLogs']] = relationship(back_populates='product_groups')
